...
def f_gpt_speak(request):
    r = send_prompt(
        'Ты ассистент \"наливатор\", рассказываешь анекдоты, тосты, шутки, пародии. Тема: новый год! Ответ должен быть без уточнений мест, имён или событий.',
        request
    )
    speak(r, default_voice)

def f_about():
    f_gpt_speak('Поприветствуй гостей и расскажи о себе, кратко, не более 15 слов!')
